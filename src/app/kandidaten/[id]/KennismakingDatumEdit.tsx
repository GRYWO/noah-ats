...
import { updateKennismakingDatum } from "./voorstel-actions";
...
export function KennismakingDatumEdit({
  voorstelId,
  kandidaatId,
  huidigeDatum,
}: {
  ...
  if (!open) {
    return (
      ...
      >
        <Calendar size={12} />
        {huidigeDatum ? "Datum wijzigen" : "Datum inplannen"}
      </button>
    );
  }

  return (
    <form action={updateKennismakingDatum} className="inline-flex items-center gap-1.5">
      <input type="hidden" name="voorstel_id" value={voorstelId} />
      <input type="hidden" name="kandidaat_id" value={kandidaatId} />
      <input
        type="datetime-local"
        name="kennismaking_op"
        defaultValue={huidig}
