...
import { tekenAkkoord } from "./actions";
...
export function TekenForm({ token, defaultNaam = "" }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    fd.set("handtekening_data", data);
    fd.set("akkoord", "on");

    startTransition(async () => {
      const r = await tekenAkkoord(fd);
      setBezig(false);
      ...
      <div className="inline-flex border-b border-[#d4d7f5] mb-0">
        <button
          ...
          className={`px-5 py-2 text-sm font-bold inline-flex items-center gap-2 ${
            tab === "typed" ? "bg-white border border-[#d4d7f5] border-b-0 rounded-t-lg text-[#333399]" : "text-gray-500 hover:text-gray-700"
          }`}
        ><Type size={14} /> Typ je naam</button>
        <button
          type="button"
          onClick={() => setTab("drawn")}
          className={`px-5 py-2 text-sm font-bold inline-flex items-center gap-2 ${
            tab === "drawn" ? "bg-white border border-[#d4d7f5] border-b-0 rounded-t-lg text-[#333399]" : "text-gray-500 hover:text-gray-700"
          }`}
        ><PenTool size={14} /> Teken handmatig</button>
      </div>
      ...
      <div className="bg-white border border-[#d4d7f5] rounded-r-lg rounded-bl-lg p-5">
        <div className="mb-3">
          ...
          <input
            ...
            required
            placeholder="bv. Pieter de Vries"
            className={tab === "typed"
        ...
        {tab === "drawn" && (
          <div className="mb-3">
            ...
            <button type="button" onClick={clearCanvas} className="mt-1 text-xs text-gray-500 hover:text-[#333399]">Wissen</button>
          </div>
        )}

        <label className="flex items-start gap-2 text-xs text-gray-700 py-3">
          <input type="checkbox" checked={akkoord} onChange={(e) => setAkkoord(e.target.checked)} className="mt-0.5 accent-[#333399]" />
          <span>Ik ga akkoord met de bovenstaande overeenkomst en bevestig dat ik de inhoud begrepen heb.</span>
        </label>
